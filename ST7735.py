...
# with some modifications to support vietnamese
...
class TFT(object):
    ...
    def char(self, aPos, aChar, aColor, aFont, aSizes):
        ...
        org_char = aChar
        aChar = vietnamese.to_ascii(aChar)

        # draw accents
        # note that this only works for font width == 5
        if aChar in "ueoaiydUEOAIYD" and aChar != org_char:
            accents = vietnamese.ACCENT_SYMBOL_MAP[org_char]
            stack_above = 0
            for accent in accents:
                bitmap = vietnamese.ACCENT_BITMAP_MAP[accent]
                bitmap_positition = vietnamese.ACCENT_POSITION_MAP[accent]

                py = 0
                if bitmap_positition == -1:
                    py += aFont["Height"] * aSizes[1] + 1
                elif bitmap_positition == 1:
                    py -= len(bitmap) * aSizes[1] + 1 - stack_above
                    if accent == "horn":
                        py += 1
                    else:
                        stack_above += py
                ...
                for bits in bitmap:
                    ...
                    if aChar == 'D':
                        px -= aSizes[0]
                    elif aChar == 'u' or aChar == 'U':
                        px += aSizes[0]

                    for i in range(5):  # because accent width is 5
                        if (bits >> (5 - i - 1)) & 1:
    ...
    def fillrect(self, aStart, aSize, aColor):
        ...
        start = (clamp(aStart[0], 0, self._size[0]), clamp(aStart[1], 0, self._size[1]))
            ...
            clamp(start[0] + aSize[0] - 1, 0, self._size[0]),
    ...
    def initb(self):
        ...
        data2[
            0
        ] = 0x15  # 1 clock cycle nonoverlap, 2 cycle gate rise, 3 cycle oscil, equalize
